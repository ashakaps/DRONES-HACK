...
- **Загрузка в базу данных**.  Очещённые данные загружаются в PostgreSQL с расширением PostGIS.  Схема базы предусматривает таблицы регионов, типов БПЛА, фактов полётов, геоточек, метрик, погоды и вспомогательных измерений【285338895072870†L3-L96】.
- **REST‑API**.  Веб‑сервер на FastAPI предоставляет следующие энд‑точки:
  - `/health` — проверка состояния сервиса【332329597938983†L33-L36】.
  - `/geo/regions` и `/geo/cities` — выдача геометрий регионов и городов в формате GeoJSON, полученных напрямую из PostGIS【460859592111394†L6-L47】.
  - `/charts/{name}` — генерация графиков (PNG) по заранее определённым сценариям: недельная активность по выбранному городу, суммарная месячная активность и распределение топ‑10 регионов【480593006845547†L6-L28】.
...
Docker‑композиция (`docker-compose.yml`) описывает три сервиса: веб‑API, микросервис ML и базу данных PostGIS.  API публикует порт `8080`, ML‑сервис — порт `8001`, база данных — `5432`【699770784845106†L0-L44】.  Сервис API монтирует исходники (`/app`) и зависит от ML и DB сервисов для корректной работы.
...
| Путь                     | Назначение |
|--------------------------|------------|
| `app/main.py`            | Инициализация FastAPI, настройка CORS и маршрутов【332329597938983†L24-L41】 |
| `app/routes/geo.py`      | Энд‑точки для получения геоданных о регионах и городах【460859592111394†L6-L47】 |
| `app/routes/charts.py`   | Энд‑точка для генерации графиков и выдачи путей к файлам PNG【480593006845547†L6-L28】 |
...
| `database/schema_db.sql` | SQL‑схема базы данных PostGIS【285338895072870†L3-L96】 |
...
| `src/preprocessing.py`   | Нормализация и объединение сырых данных по годам【136405071338125†L59-L96】 |
...
| `docker-compose.yml`     | Описание сервисов Docker【699770784845106†L0-L44】 |
| `requirements.txt`       | Список зависимостей Python【208941733064256†L0-L13】 |
...
Зависимости Python перечислены в `requirements.txt`: `fastapi`, `uvicorn`, `pydantic`, `asyncpg`, `python-dotenv`, инструменты для работы с данными (`pandas`, `openpyxl`), построения графиков (`matplotlib`, `seaborn`) и работы с базой (`sqlalchemy`, `psycopg-binary`)【208941733064256†L0-L13】.
